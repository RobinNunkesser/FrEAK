 ...
 * problem which takes into account the number of connected components and
 * the weights of the selected edges in the graph.
 */
public class MSTWithComponentCountOnly extends AbstractMSTFitnessFunction {
	
	/**
	 * Creates a new instance of MSTWithComponentAndEdgeCount
	 */
	public MSTWithComponentCountOnly(Schedule schedule) {
		super(schedule);
	}
	...
	public double evaluate(Genotype gen) {
		...
		return (-((numOfComponents - 1) * wub + edgeWeightSum));
	}
	...
	public String getDescription() {
		...
		" and the sum of the weight of all edges in E'."
		;
	}
	
	public String getName() {
		return ("MST with components only");
	}
	...
	public int compareIndividuals(Individual ind1, Individual ind2) {
		...
		// components match... compare edge weights
